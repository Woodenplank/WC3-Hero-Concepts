...
BlzSetEventDamage()
    ...
    In 1.31 PTR there’s currently 3 new damage events:
        ...
        EVENT_UNIT_DAMAGED          - old classic event for a specific unit;

        EVENT_PLAYER_UNIT_DAMAGED   - Same as 1, but for all units of a specific player on the map;

        // This seems to work fine anyway:
            TriggerRegisterAnyUnitEventBJ(gg_trg_a, EVENT_PLAYER_UNIT_DAMAGING)

        EVENT_UNIT_DAMAGING         - triggers before any armor, armor type and other resistances. Event for a specific unit like 1.
